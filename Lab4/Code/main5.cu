...
double CPU_Histo = 0, CPU_Eq = 0, CPU_sum = 0,GPU_Histo = 0, GPU_Eq = 0, GPU_sum = 0;
...
int main(int argc, char *argv[]){
    ...
    fprintf(ofp, "------------------------------------------------------------\n");
    fprintf(ofp, "CPU Histo time: \t%lf s\n", CPU_Histo/100);
    fprintf(ofp, "CPU EQ time: \t\t%lf s\n", CPU_Eq/100);
    fprintf(ofp, "CPU Total time: \t%lf s\n", CPU_sum/100);
    fprintf(ofp, "------------------------------------------------------------\n");
    fprintf(ofp, "GPU Histo time: \t%lf s\n", GPU_Histo/100);
    fprintf(ofp, "GPU Eq time: \t\t%lf s\n", GPU_Eq/100);
    fprintf(ofp, "GPU Total time: \t%lf s\n", GPU_sum/100);
    fprintf(ofp, "------------------------------------------------------------\n");
    fprintf(ofp, "GPU is %f faster than CPU\n", (CPU_sum/100)/(GPU_sum/100));
    fprintf(ofp, "------------------------------------------------------------\n");
...
void run_cpu_gray_test(PGM_IMG img_in, char *out_filename)
{
    struct timespec  tv1, tv2, tv3, tv4, tv5, tv6;
    ...
    */
    ...
    CPU_sum += (double) ((double) (tv2.tv_nsec - tv1.tv_nsec) / 1000000000.0 + (double) (tv2.tv_sec - tv1.tv_sec));
    CPU_Histo += (double) ((double) (tv4.tv_nsec - tv1.tv_nsec) / 1000000000.0 + (double) (tv4.tv_sec - tv1.tv_sec));
    CPU_Eq += (double) ((double) (tv2.tv_nsec - tv4.tv_nsec) / 1000000000.0 + (double) (tv2.tv_sec - tv4.tv_sec));
    ...
    */
    ...
    GPU_sum += (double) (((elapsedTime + elapsedTime2) / 1000) + ((double) (tv6.tv_nsec - tv5.tv_nsec) / 1000000000.0 +
           (double) (tv6.tv_sec - tv5.tv_sec)));
    GPU_Histo += (double) ((elapsedTime2 / 1000) + ((double) (tv6.tv_nsec - tv5.tv_nsec) / 1000000000.0 +
           (double) (tv6.tv_sec - tv5.tv_sec)));
    GPU_Eq += (double) (elapsedTime / 1000);
