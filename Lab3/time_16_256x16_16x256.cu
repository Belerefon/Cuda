...
//Kernel gia efarmogh filtrou kata grammes
 ...
 void convolutionRowGPU(float *d_Buffer, float *d_Input, int imageW, int imageH) {
	...
	int x = blockIdx.x * 16*blockDim.x + threadIdx.x;
	...
	data[data_reg + 6*blockDim.x] = d_Input[reg + filter_radius + 4*blockDim.x];
	data[data_reg + 7*blockDim.x] = d_Input[reg + filter_radius + 5*blockDim.x];
	data[data_reg + 8*blockDim.x] = d_Input[reg + filter_radius + 6*blockDim.x];
	data[data_reg + 9*blockDim.x] = d_Input[reg + filter_radius + 7*blockDim.x];

	data[data_reg + 10*blockDim.x] = d_Input[reg + filter_radius + 8*blockDim.x];
	data[data_reg + 11*blockDim.x] = d_Input[reg + filter_radius + 9*blockDim.x];
	data[data_reg + 12*blockDim.x] = d_Input[reg + filter_radius + 10*blockDim.x];
	data[data_reg + 13*blockDim.x] = d_Input[reg + filter_radius + 11*blockDim.x];
	data[data_reg + 14*blockDim.x] = d_Input[reg + filter_radius + 12*blockDim.x];
	data[data_reg + 15*blockDim.x] = d_Input[reg + filter_radius + 13*blockDim.x];
	data[data_reg + 16*blockDim.x] = d_Input[reg + filter_radius + 14*blockDim.x];
	data[data_reg + 17*blockDim.x] = d_Input[reg + filter_radius + 15*blockDim.x];
	...
	for (k = -filter_radius; k <= filter_radius; k++) {
		...
		sum5 += data[data_reg + 4*blockDim.x + filter_radius + k] * d_Filter[filter_radius - k];
		sum6 += data[data_reg + 5*blockDim.x + filter_radius + k] * d_Filter[filter_radius - k];
		sum7 += data[data_reg + 6*blockDim.x + filter_radius + k] * d_Filter[filter_radius - k];
		sum8 += data[data_reg + 7*blockDim.x + filter_radius + k] * d_Filter[filter_radius - k];

		sum9 += data[data_reg + 8*blockDim.x + filter_radius + k] * d_Filter[filter_radius - k];
		sum10 += data[data_reg + 9*blockDim.x + filter_radius + k] * d_Filter[filter_radius - k];
		sum11 += data[data_reg + 10*blockDim.x + filter_radius + k] * d_Filter[filter_radius - k];
		sum12 += data[data_reg + 11*blockDim.x + filter_radius + k] * d_Filter[filter_radius - k];
		sum13 += data[data_reg + 12*blockDim.x + filter_radius + k] * d_Filter[filter_radius - k];
		sum14 += data[data_reg + 13*blockDim.x + filter_radius + k] * d_Filter[filter_radius - k];
		sum15 += data[data_reg + 14*blockDim.x + filter_radius + k] * d_Filter[filter_radius - k];
		sum16 += data[data_reg + 15*blockDim.x + filter_radius + k] * d_Filter[filter_radius - k];
	}
	...
	d_Buffer[reg + 4*blockDim.x] = sum5;
	d_Buffer[reg + 5*blockDim.x] = sum6;
	d_Buffer[reg + 6*blockDim.x] = sum7;
	d_Buffer[reg + 7*blockDim.x] = sum8;

	d_Buffer[reg + 8*blockDim.x] = sum9;
	d_Buffer[reg + 9*blockDim.x] = sum10;
	d_Buffer[reg + 10*blockDim.x] = sum11;
	d_Buffer[reg + 11*blockDim.x] = sum12;
	d_Buffer[reg + 12*blockDim.x] = sum13;
	d_Buffer[reg + 13*blockDim.x] = sum14;
	d_Buffer[reg + 14*blockDim.x] = sum15;
	d_Buffer[reg + 15*blockDim.x] = sum16;
...
//Kernel gia efarmogh filtrou kata sthles
 ...
 void convolutionColumnGPU(float *d_Output, float *d_Buffer, int imageW, int imageH) {
	...
	int y = blockIdx.y * 16*blockDim.y + threadIdx.y;
	...
	data[data_reg + 10*blockDim.y * TILE_W2] = d_Buffer[reg + (imageW + 2*filter_radius) * (filter_radius + 8*blockDim.x)];
	data[data_reg + 11*blockDim.y * TILE_W2] = d_Buffer[reg + (imageW + 2*filter_radius) * (filter_radius + 9*blockDim.x)];
	data[data_reg + 12*blockDim.y * TILE_W2] = d_Buffer[reg + (imageW + 2*filter_radius) * (filter_radius + 10*blockDim.x)];
	data[data_reg + 13*blockDim.y * TILE_W2] = d_Buffer[reg + (imageW + 2*filter_radius) * (filter_radius + 11*blockDim.x)];
	data[data_reg + 14*blockDim.y * TILE_W2] = d_Buffer[reg + (imageW + 2*filter_radius) * (filter_radius + 12*blockDim.x)];
	data[data_reg + 15*blockDim.y * TILE_W2] = d_Buffer[reg + (imageW + 2*filter_radius) * (filter_radius + 13*blockDim.x)];
	data[data_reg + 16*blockDim.y * TILE_W2] = d_Buffer[reg + (imageW + 2*filter_radius) * (filter_radius + 14*blockDim.x)];
	data[data_reg + 17*blockDim.y * TILE_W2] = d_Buffer[reg + (imageW + 2*filter_radius) * (filter_radius + 15*blockDim.x)];
	...
	for (int k = -filter_radius; k <= filter_radius; k++) {
		...
		sum9 += data[data_reg + (8*blockDim.y + k + filter_radius) * TILE_W2] * d_Filter[filter_radius - k];
		sum10 += data[data_reg + (9*blockDim.y + k + filter_radius) * TILE_W2] * d_Filter[filter_radius - k];
		sum11 += data[data_reg + (10*blockDim.y + k + filter_radius) * TILE_W2] * d_Filter[filter_radius - k];
		sum12 += data[data_reg + (11*blockDim.y + k + filter_radius) * TILE_W2] * d_Filter[filter_radius - k];
		sum13 += data[data_reg + (12*blockDim.y + k + filter_radius) * TILE_W2] * d_Filter[filter_radius - k];
		sum14 += data[data_reg + (13*blockDim.y + k + filter_radius) * TILE_W2] * d_Filter[filter_radius - k];
		sum15 += data[data_reg + (14*blockDim.y + k + filter_radius) * TILE_W2] * d_Filter[filter_radius - k];
		sum16 += data[data_reg + (15*blockDim.y + k + filter_radius) * TILE_W2] * d_Filter[filter_radius - k];
	...
	d_Output[(y + 8*blockDim.y) * imageW + x] = sum9;
	d_Output[(y + 9*blockDim.y) * imageW + x] = sum10;
	d_Output[(y + 10*blockDim.y) * imageW + x] = sum11;
	d_Output[(y + 11*blockDim.y) * imageW + x] = sum12;
	d_Output[(y + 12*blockDim.y) * imageW + x] = sum13;
	d_Output[(y + 13*blockDim.y) * imageW + x] = sum14;
	d_Output[(y + 14*blockDim.y) * imageW + x] = sum15;
	d_Output[(y + 15*blockDim.y) * imageW + x] = sum16;
...
int main(int argc, char **argv) {
	...
	*d_Output;
    ...
    ofp = fopen("Output_16_256x16_16x256.txt","w");
