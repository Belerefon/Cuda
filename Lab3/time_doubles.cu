...
///Kernel gia efarmogh filtrou kata grammes
 __global__
 void convolutionRowGPU(double *d_Buffer, double *d_Input, int imageW, int imageH) {

	__shared__ double data[TILE_H1 * (TILE_W1 + 2*filter_radius)];
...
//Kernel gia efarmogh filtrou kata sthles
 __global__
 void convolutionColumnGPU(double *d_Output, double *d_Buffer, int imageW, int imageH) {

	__shared__ double data[TILE_W2 * (TILE_H2 + 2*filter_radius)];
...
int main(int argc, char **argv) {
	...
	*d_Output;
    ...
    ofp = fopen("Output_doubles.txt","w");
    ...
    for (int k = 1; k < 9 ; k++) {
		...
		for (int j = 1; j < 13; j++) {
			...
			h_Input_padding 	= (double *)malloc((imageW + 2*filter_radius) * (imageH + 2*filter_radius) * sizeof(double));
			...
			gpuErrchk( cudaMalloc((void**)&d_Input, (imageW + 2*filter_radius) * (imageH + 2*filter_radius) * sizeof(double)) );
			gpuErrchk( cudaMalloc((void**)&d_Buffer, (imageW + 2*filter_radius) * (imageH + 2*filter_radius) * sizeof(double)) );
			gpuErrchk( cudaMalloc((void**)&d_Output, imageW * imageH * sizeof(double)) );
			...
			gpuErrchk( cudaMemcpy(d_Input, h_Input_padding, (imageW + 2*filter_radius) * (2*filter_radius + imageW) * sizeof(double), cudaMemcpyHostToDevice) );
			gpuErrchk( cudaMemset(d_Buffer, 0, (imageW + 2*filter_radius) * (2*filter_radius + imageW) * sizeof(double)) );
			...
			gpuErrchk( cudaMemcpy(h_OutputGPU, d_Output,  imageW * imageH * sizeof(double), cudaMemcpyDeviceToHost) );
