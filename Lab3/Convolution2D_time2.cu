...
#define TILE_W1 16
#define TILE_H1 TILE_WIDTH
#define TILE_W2 TILE_H1
#define TILE_H2 TILE_W1
#define repeat ((TILE_H2 + 2*filter_radius) / TILE_WIDTH)
...
__device__ __constant__ float d_Filter[FILTER_LENGTH];
...
//Kernel gia efarmogh filtrou kata grammes
 ...
 void convolutionRowGPU(float *d_Buffer, float *d_Input, int imageW, int imageH) {
	...
	for ( int i=1; i<repeat; i++) {
		data[threadIdx.x + threadIdx.y * (TILE_W1 + 2*filter_radius)] = d_Input[(y + filter_radius) * (imageW + 2*filter_radius) + (x + filter_radius) - filter_radius];
		data[threadIdx.x + i*blockDim.x + threadIdx.y * (TILE_W1 + 2*filter_radius)] = d_Input[(y + filter_radius) * (imageW + 2*filter_radius) + (x + filter_radius) + (i-1)*filter_radius];
	}
	...
	for (k = -filter_radius; k <= filter_radius; k++)
		sum += data[threadIdx.y * (TILE_W1 + 2*filter_radius) + threadIdx.x + filter_radius + k] * d_Filter[filter_radius - k];
        
	d_Buffer[(y + filter_radius) * (imageW + 2*filter_radius) + (x + filter_radius)] = sum;
 }
...
__global__
 void convolutionColumnGPU(float *d_Output, float *d_Buffer, int imageW, int imageH) {
	...
	for ( int i=1; i<repeat; i++) {
		data[threadIdx.x + threadIdx.y * TILE_W2] = d_Buffer[(y + filter_radius) * (imageW + 2*filter_radius) + (x + filter_radius) - (imageW + 2*filter_radius) * filter_radius];	//data[threadIdx.x + threadIdx.y * TILE_W + blockDim.y * TILE_W] = d_Buffer[(y + filter_radius) * (imageW + 2*filter_radius) + (x + filter_radius)];
		data[threadIdx.x + threadIdx.y * TILE_W2 + i*blockDim.y * TILE_W2] = d_Buffer[(y + filter_radius) * (imageW + 2*filter_radius) + (x + filter_radius) + (i-1)*(imageW + 2*filter_radius) * filter_radius];
	}
	...
	float sum = 0;
	for (k = -filter_radius; k <= filter_radius; k++)
		sum += data[(threadIdx.y + k + filter_radius) * TILE_W2 + threadIdx.x] * d_Filter[filter_radius - k];
...
int main(int argc, char **argv) {
	...
	*d_Output;
    ...
    ofp = fopen("Output20.txt","w");
	
	int tile_width = TILE_WIDTH;
	int tile_w1 = TILE_W1;
	int tile_h1 = TILE_H1;
	
	fprintf(ofp,"TILE_WIDTH: %d\nTILE_W1: %d\nTILE_H1: %d\n-------------------\n", tile_width, tile_w1, tile_h1);
    ...
    for (int k = 1; k < 9 ; k++) {
		...
		for (int j = 1; j < 21; j++) {
			...
			convolutionRowGPU<<<dimGrid, dimBlock>>>(d_Buffer, d_Input, imageW, imageH);

			gpuErrchk( cudaPeekAtLastError() );
			
			
			//Kaloume ton deutero Kernel
			convolutionColumnGPU<<<dimGrid, dimBlock>>>(d_Output, d_Buffer, imageW, imageH);
