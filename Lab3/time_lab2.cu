...
//Kernel gia efarmogh filtrou kata grammes
 __global__
 void convolutionRowGPU(float *d_Buffer, float *d_Input, float *d_Filter, int imageW, int imageH) {
	int k;
	float sum = 0;
...
__global__
 void convolutionColumnGPU(float *d_Dst, float *d_Buffer, float *d_Filter, int imageW, int imageH) {
	int k;
	float sum = 0;
	...
	*d_Buffer;
    ...
    for (int k = 1; k < 9 ; k++) {
		...
		for (int j = 1; j < 13; j++) {
			...
			convolutionRowGPU<<<dimGrid, dimBlock>>>(d_Buffer, d_Input, d_Filter, imageW, imageH);

			gpuErrchk( cudaPeekAtLastError() );

			
			//Kaloume ton deutero Kernel
			convolutionColumnGPU<<<dimGrid, dimBlock>>>(d_Dst, d_Buffer, d_Filter, imageW, imageH);
