...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden)
{
	...
	for  (i=1; i<SIZE-1; i+=1 ) {
		...
		for (j=1; j<SIZE-1; j+=1) {
			 ...
			 * of the derivative.								  */
			res1 = 0;
			res1 += input[(i + -1)*SIZE + j + -1] * horiz_operator[0][0];
			res1 += input[(i + 0)*SIZE + j + -1] * horiz_operator[1][0];
			res1 += input[(i + 1)*SIZE + j + -1] * horiz_operator[2][0];
			res1 += input[(i + -1)*SIZE + j + 0] * horiz_operator[0][1];
			res1 += input[(i + 0)*SIZE + j + 0] * horiz_operator[1][1];
			res1 += input[(i + 1)*SIZE + j + 0] * horiz_operator[2][1];
			res1 += input[(i + -1)*SIZE + j + 1] * horiz_operator[0][2];
			res1 += input[(i + 0)*SIZE + j + 1] * horiz_operator[1][2];
			res1 += input[(i + 1)*SIZE + j + 1] * horiz_operator[2][2];
			res2 = 0;
			res2 += input[(i + -1)*SIZE + j + -1] * vert_operator[0][0];
			res2 += input[(i + 0)*SIZE + j + -1] * vert_operator[1][0];
			res2 += input[(i + 1)*SIZE + j + -1] * vert_operator[2][0];
			res2 += input[(i + -1)*SIZE + j + 0] * vert_operator[0][1];
			res2 += input[(i + 0)*SIZE + j + 0] * vert_operator[1][1];
			res2 += input[(i + 1)*SIZE + j + 0] * vert_operator[2][1];
			res2 += input[(i + -1)*SIZE + j + 1] * vert_operator[0][2];
			res2 += input[(i + 0)*SIZE + j + 1] * vert_operator[1][2];
			res2 += input[(i + 1)*SIZE + j + 1] * vert_operator[2][2];
	...
	for (i=1; i<SIZE-1; i++) {
		...
		for ( j=1; j<SIZE-1; j+=7 ) {
			...
			PSNR += t;
			t = pow((output[temp+j+1] - golden[temp+j+1]),2);
			PSNR += t;
			t = pow((output[temp+j+2] - golden[temp+j+2]),2);
			PSNR += t;
			t = pow((output[temp+j+3] - golden[temp+j+3]),2);
			PSNR += t;
			t = pow((output[temp+j+4] - golden[temp+j+4]),2);
			PSNR += t;
			t = pow((output[temp+j+5] - golden[temp+j+5]),2);
			PSNR += t;
			t = pow((output[temp+j+6] - golden[temp+j+6]),2);
			PSNR += t;
		}
	}
